...
function create() {
    ...
    // --- ROOM 1: THE LIBRARY (ปรับกระจายหนังสือให้กว้างขึ้นและกึ่งกลางพอดี) ---
    this.add.text(40, 30, "ห้องที่ 1: ห้องสมุด (คำถาม: น้ำกับไฟใครจะชนะ?)", { font: "16px Arial", fill: "#88ccff" });
    for (let i = 0; i < 5; i++) {
        let bookX = 80 + (i * 80); // กระจายจากพิกัด 80 ถึง 400 พิกเซล
        ...
        // ใช้ setOrigin(0.5) เพื่อให้ข้อความอยู่ตรงกลางใต้หนังสือพอดี
    ...
    // --- ROOM 4: NPC & CHEST ROOM (แก้ไขขยับตำแหน่งหัวข้อพิกัด Y เพื่อไม่ให้ทับเส้นสีทอง) ---
