...
function create() {
    ...
    function roomLabel(x, y, w, text, color) {
        scene.add.rectangle(x, y, w, 22, 0x000000, 0.55).setOrigin(0, 0.5).setStrokeStyle(1, color, 0.6);
        scene.add.text(x + 8, y, text, { fontSize: '13px', fill: color, fontStyle: 'bold' }).setOrigin(0, 0.5);
    }
    roomLabel(28,  350, 270, '📖 Mission 1: Forbidden Library', '#e8c667');
    roomLabel(28,  30,  230, '🗿 Mission 2: Statue Room',       '#9fd3f0');
    roomLabel(448, 30,  220, '🔥 Mission 3: Torch Room',        '#ffb066');
    roomLabel(448, 350, 320, '🗝️ Mission 4 & 5: Trials End',   '#d9b3ff');
    ...
    scene.add.text(195, 285, 'LOCKED', { fontSize: '12px', fill: '#ff4444', fontStyle: 'bold' }).setOrigin(0.5, 1).setName('lbl_book');
    scene.add.text(420, 135, 'LOCKED', { fontSize: '12px', fill: '#ff4444', fontStyle: 'bold' }).setOrigin(0, 0.5).setName('lbl_statue');
    scene.add.text(615, 285, 'LOCKED', { fontSize: '12px', fill: '#ff4444', fontStyle: 'bold' }).setOrigin(0.5, 1).setName('lbl_torch');

    // ══ MISSION 1 – หนังสือ 5 เล่ม (bottom-left) ══
    scene.add.text(28, 368, 'Click a book to read it', { fontSize: '11px', fill: '#bbb' });
    scrollIcon = scene.add.text(28, 560, '', { fontSize: '14px', fill: '#cc88ff', fontStyle: 'bold' }).setDepth(10);
    ...
    bookData.forEach(b => {
        ...
        scene.add.text(b.x, b.y + 30, b.title,
            { fontSize: '9px', fill: '#eee', align: 'center', wordWrap: { width: 66 }, lineSpacing: 2 })
            .setOrigin(0.5, 0);
    ...
    const dirLabels = ['↑','→','↓','←'];
    scene.add.text(28, 45, 'Rotate to:  →  ↑  ←  ↓', { fontSize: '12px', fill: '#bbb' });
    ...
    statuePos.forEach((pos, i) => {
        ...
        const arrow = scene.add.text(pos.x, pos.y + 34, dirLabels[0],
            { fontSize: '16px', fill: '#ffff00', fontStyle: 'bold' }).setOrigin(0.5);
        let angle = 0;
    ...
    scene.add.text(448, 45, 'Light order:  1 → 3 → 2 → 4', { fontSize: '12px', fill: '#bbb' });
    ...
    torchPos.forEach((pos, i) => {
        ...
        scene.add.text(pos.x + 24, pos.y - 32, String(i + 1),
            { fontSize: '14px', fill: '#ccc', fontStyle: 'bold' }).setOrigin(0.5);
        const litT = scene.add.text(pos.x, pos.y + 36, 'OFF',
            { fontSize: '11px', fill: '#999', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    scene.add.text(510, 464, 'Elder Moren', { fontSize: '12px', fill: '#ccc', fontStyle: 'bold' }).setOrigin(0.5, 0);
    ...
    dialogueBox  = scene.add.rectangle(400, 560, 780, 80, 0x111133, 0.95).setVisible(false).setDepth(10);
    dialogueName = scene.add.text(25, 526, '', { fontSize: '14px', fill: '#ffff88', fontStyle: 'bold' }).setVisible(false).setDepth(11);
    dialogueText = scene.add.text(25, 546, '', { fontSize: '13px', fill: '#fff', wordWrap: { width: 630 }, lineSpacing: 4 }).setVisible(false).setDepth(11);
    nextBtn = scene.add.text(695, 526, '[ NEXT ▶ ]', { fontSize: '13px', fill: '#88ffff', fontStyle: 'bold' }).setVisible(false).setDepth(11).setInteractive();
    ...
    scene.add.text(608, 368, 'Choose a chest wisely...', { fontSize: '11px', fill: '#bbb' });
    ...
    chestData.forEach(c => {
        ...
        img.on('pointerdown', () => {
            ...
            if (c.correct) {
                ...
                scene.add.text(750, 470, 'EXIT OPEN!', { fontSize: '11px', fill: '#00ff00', align: 'center', fontStyle: 'bold' }).setOrigin(0.5);
            } else {
    ...
    // ── Win Zone ──
    scene.add.text(760, 402, 'EXIT', { fontSize: '11px', fill: '#00ff00', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    // ── Message popup ──
    msgBox  = scene.add.rectangle(400, 295, 520, 74, 0x000000, 0.92).setVisible(false).setDepth(20);
    msgText = scene.add.text(400, 295, '', { fontSize: '15px', fill: '#fff', wordWrap: { width: 490 }, align: 'center', lineSpacing: 4 })
