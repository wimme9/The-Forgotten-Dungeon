...
function create() {
    ...
    nextBtn.on('pointerdown', () => {
        ...
        if (dialogueState < dialogues.length) {
            ...
            if (!hasTalkedToNPC) {
                hasTalkedToNPC = true;
                showMsg(scene, '🗝️ Elder Moren has told you about the chests. You may now choose one.', 0x222244);
            }
        }
    });
    ...
    chestData.forEach(c => {
        ...
        img.on('pointerover', () => { if (!img.getData('opened')) img.setTint(hasTalkedToNPC ? 0xffdd88 : 0x666666); });
        img.on('pointerout',  () => { if (!img.getData('opened')) img.clearTint(); });
        img.on('pointerdown', () => {
            if (img.getData('opened')) return;
            if (!hasTalkedToNPC) {
                showMsg(scene, '💬 Speak with Elder Moren first before touching the chests.', 0x333333);
                return;
            }
