...
let npc;
let chests; // เปลี่ยนจาก chest เดี่ยวเป็น Group ของหีบทั้ง 3 ใบ
let exitDoor;
let exitDoorCollider; 
let openedChestsCount = 0; // ตัวนับจำนวนหีบที่เปิดแล้ว
...
function create() {
    ...
    npc = this.physics.add.staticSprite(450, 400, 'npc'); // ขยับหลบหีบขึ้นไปนิดหน่อย
    npc.setDisplaySize(40, 64);
    npc.setInteractive();
    npc.body.setSize(40, 64);

    // 🛠️ ปรับปรุง: สร้างหีบสมบัติ 3 ใบ เรียงแถวระนาบเดียวกันในห้องขวาล่าง
    ...
    chestPositions.forEach(pos => {
        ...
        // ดักกรณีผู้เล่นไปคลิกที่หีบตรงๆ
        c.on('pointerdown', () => {
            if (isCutsceneActive) return;
    ...
    npc.on('pointerdown', () => {
        ...
        if (openedChestsCount < 3) {
            ...
            if (npcClickCount === 1) {
                ...
                cutsceneStep = 1; // เริ่ม Cutscene ขั้นแรก (เดินไปหาหีบใบที่ 1)
                npcClickCount = 0; 
            }
        }
    });
    ...
    this.physics.add.collider(player, chests); // ชนกลุ่มหีบ
    ...
    popupText = this.add.text(120, 500, '', {
        fontSize: '18px',
        fill: '#ffffff',
...
function update() {
    ...
    if (isCutsceneActive) {
        ...
        if (cutsceneStep === 1) {
            ...
            // รอให้ข้อความหีบ 1 หายไปก่อนเดินต่อ
            if (!isShowingImportantMessage) {
                cutsceneStep = 4;
            }
        }
            ...
            // รอให้ข้อความหีบ 2 หายไปก่อนเดินต่อ
            if (!isShowingImportantMessage) {
                cutsceneStep = 6;
            }
        }
            ...
            if (distance > 5) {
                ...
                // เมื่อเปิดครบ 3 หีบ ประตูจึงจะเปิดออก
                isDoorOpened = true;
                ...
                // เปลี่ยนเป้าหมายเดินทะลุผ่านประตู
                cutsceneStep = 7;
                targetX = exitDoor.x;
                targetY = exitDoor.y;
            }
            ...
            // รอข้อความหีบ 3 หายก่อน เดินออกจากประตูบานสุดท้าย
            if (!isShowingImportantMessage) {
                cutsceneStep = 8;
            }
        }
        else if (cutsceneStep === 8) {
            // เดินเข้าหาประตูทางออก
            let distance = Phaser.Math.Distance.Between(player.x, player.y, targetX, targetY);
            ...
            if (distance > 5) {
                ...
                // ค่อยๆ หายตัวเมื่อตัวละครเดินผ่านกรอบประตู
                if (distance < 40) {
                    player.setAlpha(distance / 40);
                }
            } else {
                player.setVelocity(0, 0);
                player.setVisible(false); // ซ่อนผู้เล่น
                
                hideDialogue();
                showVictoryScreen(); // ขึ้นจอจบแบบอลังการ
    ...
    // ตรวจสอบระยะห่างจากหีบใบใดใบหนึ่งใน Group
